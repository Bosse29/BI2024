...
```{r}
weights <- train$SALE_PRC / max(train$SALE_PRC)
```
...
rf_model <- ranger(
  formula = log_SALE_PRC ~ .,     # Formula
  data = train,               # Training data
  num.trees = 500,            # Number of trees
  mtry = NULL, # Number of features per split
  min.node.size = 5,          # Minimum size of terminal nodes
  max.depth = 25,
  case.weights = weights  # Maximum tree depth
)
```

```{r}
rf_pred <- exp(predict(rf_model, data = valid)$predictions)


plot(valid$SALE_PRC, rf_pred)
abline(0,1,col = "red", lwd = 2)

# Calculate RMSE (Root Mean Squared Error)
rmse <- sqrt(mean((rf_pred - valid$SALE_PRC)^2))
cat("RMSE:", rmse, "\n")
```

```{r}
rf_pred <- exp(predict(rf_model, data = test)$predictions)
