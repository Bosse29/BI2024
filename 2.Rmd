...
```
```{r}
# Install and load the sf package if not already installed
library(sf)

# Load the shapefile
zip_shapefile <- st_read("tl_2016_us_zcta510.shp")
# Assuming your data frame is called 'data' and has 'LATITUDE' and 'LONGITUDE' column
points_sf <- st_as_sf(data, coords = c("LONGITUDE", "LATITUDE"), crs = st_crs(zip_shapefile))

# Perform a spatial join to get the ZIP code for each point
data_with_zip <- st_join(points_sf, zip_shapefile, join = st_within)

# Extract the ZIP code column (e.g., "ZCTA5CE20" is often the ZIP code column name)
data_with_zip$ZIP_CODE <- data_with_zip$ZCTA5CE20
